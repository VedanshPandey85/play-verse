  ...
  const fetchEvents = async (sport) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/events?sport=${sport}`
      );
      ...
      const sortedEvents = data.sort((a, b) => {
        ...
        const diffA = Math.abs(new Date(a.date) - currentDate); // Difference for event A
        const diffB = Math.abs(new Date(b.date) - currentDate); // Difference for event B

        // Sort by the smallest difference (closest to current date)
        return diffA - diffB;
      });

      // Fetch payment data for each event
      const paymentPromises = sortedEvents.map((event) =>
        axios
          .get(
  ...
  return (
    <div className="container mx-auto mt-20 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.map((event) => {
          const slotsLeft =
            event.participantsLimit - (successfulPayments[event._id] || 0);
          return (
            <div
              key={event._id}
